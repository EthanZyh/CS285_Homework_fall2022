...
class RL_Trainer(object):
        ...
        for itr in range(n_iter):
            ...
            if isinstance(self.agent, MBPOAgent):
                for _ in range(self.sac_params['n_iter']):
                    # print(f'rl_trainer: {_}/{self.sac_params["n_iter"]}')
                    if self.params['mbpo_rollout_length'] > 0:
                        ...
                        traj = self.agent.collect_model_trajectory(self.params['mbpo_rollout_length'])
                        self.agent.add_to_replay_buffer(traj, from_model=True)
                    # train the SAC agent
    ...
    def train_sac_agent(self):
        ...
        for _ in range(self.sac_params['num_agent_train_steps_per_iter']):
            self.agent.train_sac(*self.agent.sample_sac(self.sac_params['train_batch_size']))
        return
        all_logs = [
            self.agent.train_sac(*self.agent.sample_sac(self.sac_params['train_batch_size'])) 
            for _ in range(self.params['num_agent_train_steps_per_iter'])
