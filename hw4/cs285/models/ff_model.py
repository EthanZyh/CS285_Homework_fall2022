...
class FFModel(nn.Module, BaseModel):
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        delta_normalized = normalize(next_observations-observations, data_statistics['delta_mean'], data_statistics['delta_std'])
        delta_normalized = ptu.from_numpy(delta_normalized)
        loss = self.loss(target, delta_normalized) # TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
